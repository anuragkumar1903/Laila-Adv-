...
- **Local LLM Host**: Ollama with `qwen2.5-coder:7b`  
- **Storage**: SQLite  
...
```json
{
  ...
  "files": [
    { "path": "src/controllers/userController.ts", "role": "controller", "language": "ts", "hash": "sha256..." },
    { "path": "src/services/userService.ts", "role": "service", "language": "ts", "hash": "sha256..." }
  ],
  "routes": [
    { "method": "GET", "path": "/users", "handler": "src/controllers/userController.ts::getUsers" }
  ],
...
- Accept commands mapped to agents:
  - `laila-cli init` initialize project
  - `laila-cli scan` rescan project
  - `laila-cli run --agent coder --intent "refactor X"`
  - `laila-cli status` show projects and last scan
  - `laila-cli task status <task-id>`
...
### Refactor Function
1. `laila-cli run --agent coder --intent "refactor user service to async/await"`
...
- Run `npm run dev` and `laila-cli init` against a small sample project.
...
- Scanner misses files: adjust ignore patterns and run `laila-cli scan --depth N`.

**Helpful commands**
- Show projects: `laila-cli status`
- Rescan project: `laila-cli scan --project <id>`
- Task status: `laila-cli task status <task-id>`
