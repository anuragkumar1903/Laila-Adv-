...
## System View
Laila is a local-first CLI assistant for software engineering work. It is designed to keep context compact, execution auditable, and all runtime state local to the machine.

```mermaid
flowchart LR
  CLI[CLI / laila-cli] --> ORCH[Orchestrator]
...
2. Running `laila-cli` with no subcommand launches the interactive assistant.
...
## Operational Notes
- `laila-cli doctor` checks environment readiness.
- `laila-cli doctor --fix` can create missing local folders.
- `laila-cli skills` lists discovered skill bundles.
- `laila-cli scan` refreshes the project index.
