# Laila(Adv) AI Handoff

## What This Project Is

Laila(Adv) is a local-first AI CLI assistant for software engineering work. It runs entirely on the user's machine, uses Ollama for model inference, SQLite for persistence, and a scanned project index to keep context small and relevant.

The CLI is centered around `laila`, which now launches the interactive assistant directly when invoked with no subcommand.
...
2. `laila` with no subcommand enters the interactive start flow.
...
Laila(Adv) uses agent roles rather than a single generic prompt.
...
Laila(Adv) avoids loading full repositories into the model.
...
- `laila` - start the interactive assistant
- `laila ask "..."` - one-off question
- `laila scan [path]` - index a project
- `laila status` - show the latest session and project
- `laila history` - show recent tasks
- `laila skills` - list discovered skills
- `laila doctor` - inspect readiness and suggestions
- `laila doctor --fix` - create safe local folders when missing
